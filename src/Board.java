...
public class Board extends JPanel implements ActionListener {
    ...
    final int BALLD = 25, PADDLEW = 20, PADDLEH = 50, BORDER = 10;
    ...
    public void checkCollisions(){
        ...
        if(ball.intersects(paddle1) || ball.intersects(paddle1)){
            ballDX *=-1;
        }
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        if(MENU){
            ...
            printSimpleString("PONG", getWidth(), 0, getHeight()/3 *2, g);
            printSimpleString("PRESS P TO PLAY", getWidth(), 0, getHeight()/3 *2, g);

            }else if(PLAY){
            g.fillOval(ballX, ballY, BALLD, BALLD);
            g.fillOval(paddlePX, paddlePY, PADDLEW, PADDLEH);
            g.fillOval(paddleCX, paddleCY, PADDLEW, PADDLEH);
